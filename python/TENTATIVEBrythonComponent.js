...
const PythonGameComponent = () => {
  ...
  const runPythonCode = () => {
    ...
    # Détermination du chemin de base pour les ressources
    ...
    def resource_path(relative_path):
        """ Convert relative path to absolute path, for PyInstaller """
        base_path = getattr(sys, '_MEIPASS', os.path.dirname(os.path.abspath(__file__)))
        return os.path.join(base_path, relative_path)
    
    
    # Chargement des ressources avec des chemins relatifs
    player_images_right = {
        "normalR": pygame.image.load(resource_path('../python/src/lib/persoD.png')),
        "cuttingR1": pygame.image.load(resource_path('../python/src/lib/an_axeD.png')),
        "cuttingR2": pygame.image.load(resource_path('../python/src/lib/cutD.png'))
    }
    player_images_left = {
        "normalL": pygame.image.load(resource_path('../python/src/lib/perso.png')),
        "cuttingL1": pygame.image.load(resource_path('../python/src/lib/an_axe.png')),
        "cuttingL2": pygame.image.load(resource_path('../python/src/lib/cut.png'))
    }
    tree_image = pygame.image.load(resource_path('../python/src/lib/buche.png'))
    branch_image_right = pygame.image.load(resource_path('../python/src/lib/branche.png'))
    branch_image_left = pygame.image.load(resource_path('../python/src/lib/brancheG.png'))
    background_image = pygame.image.load(resource_path('../python/src/lib/fond.png'))
    overlay_image = pygame.image.load(resource_path('../python/src/lib/plan.png'))
    background_image_50 = pygame.image.load(resource_path('../python/src/lib/fond7.png'))
    overlay_image_50 = pygame.image.load(resource_path('../python/src/lib/plan7.png'))
    background_image_100 = pygame.image.load(resource_path('../python/src/lib/fond2.png'))
    overlay_image_100 = pygame.image.load(resource_path('../python/src/lib/plan2.png'))
    background_image_150 = pygame.image.load(resource_path('../python/src/lib/fond3.png'))
    overlay_image_150 = pygame.image.load(resource_path('../python/src/lib/plan3.png'))
    background_image_200 = pygame.image.load(resource_path('../python/src/lib/fond6.png'))
    overlay_image_200 = pygame.image.load(resource_path('../python/src/lib/plan6.png'))
    
    hit_sound = pygame.mixer.Sound(resource_path('../python/src/sound/move.mp3'))
    
    pygame.mixer.music.load(resource_path('../python/src/sound/music.mp3'))
    ...
    def read_best_score():
        try:
            with open(resource_path('src/data.txt'), 'r') as file:
                return int(file.read())
        except (IOError, ValueError):
            return 0
    
    def save_best_score(score):
        with open(resource_path('src/data.txt'), 'w') as file:
        ...
        if cut_animation_counter > 0:
            ...
            if cut_animation_counter == 5:
                ...
                if (last_branch != "left" and player_pos == "left") or (last_branch != "right" and player_pos == "right"):
                    ...
                    if len(falling_logs) < 8:  # Limiter à 8 animations simultanées
                        ...
                        falling_logs.append({
                            ...
                            "y": 588,
                            "velocity": -10 * 2,
                            "side_velocity": -10 * 2 if player_pos == "right" else 10 * 2,
                            "gravity": 1.5
                        })
    
                   # Créer l'animation de chute de la bûche
                    if len(falling_buches) < 8:  # Limiter à 8 animations simultanées
    
                        falling_buches.append({
    ...
    while True: 
        ...
        if not game_over:
            ...
            # Mettre à jour et dessiner chaque animation de chute de branche
            for falling_log in falling_logs:
                falling_log["y"] += falling_log["velocity"]
                falling_log["x"] += falling_log["side_velocity"]
                falling_log["velocity"] += falling_log["gravity"]
                if falling_log["y"] <= 590:  # Dessiner tant que l'animation est à l'écran
                    screen.blit(falling_log["image"], (falling_log["x"], falling_log["y"]))
    
            # Supprimer les animations terminées
            falling_logs = [log for log in falling_logs if log["y"] <= 590]
    
            # Faire de même pour les animations de chute de bûche
            for falling_buche in falling_buches:
                ...
                if falling_buche["y"] <= 590:
                    screen.blit(falling_buche["image"], (falling_buche["x"], falling_buche["y"]))
    
            falling_buches = [buche for buche in falling_buches if buche["y"] <= 590]
        ...
        // Capturer la sortie du code Python
        const pythonOutput = Brython.stdout().trim();
        setOutput(pythonOutput);
      };
    
      useEffect(() => {
        // Exécutez le code Python lorsque le composant est monté
        runPythonCode();
      }, []);
      ...
      return (
        <div>
          <h2>Python Game</h2>
          <button onClick={runPythonCode}>Exécuter le code Python</button>
          <div>Sortie Python: {output}</div>
        </div>
      );
    };
    
    export default PythonGameComponent;